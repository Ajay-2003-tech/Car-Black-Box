...
void init_uart(void)
{
	...
	TX9 = 0;                // It means we are not sending any parity bit only 8 bit so we are keeping it 0 else 1
	...
	SPEN = 1;                   // Serial protocol enable bit
	/* 9bit RX enable or disable bit */
	RX9 = 0;                       // it should be same as the TX9
